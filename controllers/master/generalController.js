import {dbHris, db, dbMaster } from "../../config/db.js";
 ...
 export const listParentMenu = async (req, res) => {
  ...
  try {
    ...
    if (!rowsPerPage) {
      ...
      let query = baseQuery.select('id', 'name');
      
      if (decryptedCode) {
        query = query.where('id', decryptedCode);
      }
      
      if (needle) {
        query = query.where('name', 'like', `%${needle}%`);
      }
      
      const response = await query.orderBy("name").limit(10);
    ...
    const columnSort = sortBy === 'asc' ? 'order_menu asc' : `${sortBy} ${sorting}`;
    ...
    if (filter) {
      query = query.where((subQuery) => {
        subQuery
          ...
          .orWhere('icon', 'like', `%${filter}%`)
          .orWhereRaw('CAST(order_menu AS varchar) like ?', `%${filter}%`);
      });
    }
...
export const saveParent = async (req, res) => {
  ...
  try {
    const { name, icon, order, id: encryptedId, creator: encryptedCreator } = req.body;
    ...
    const existingMenu = await trx("menu")
      .whereNull("parent")
      .where("name", name)
      .where('id', '<>', id)
      .first();
...
export const listSubMenu = async (req, res) => {
  ...
  try {
    ...
    const currentPage = Math.floor(page);
    const perPage = Math.floor(rowsPerPage);
    const parent = await decrypt(encryptedParent);
    
    let query = db("menu")
      .select("*", db.raw("(select name from menu where id= ?) as parent_name", [parent]))
      .where('parent', parent)
      .whereNull('deleted_at');
...
export const saveSubMenu = async (req, res) => {
  ...
  const trx = await db.transaction();
  try {
    const { name, link, icon, order, id: encryptedId, dechiper, parent: encryptedParent, creator: encryptedCreator } = req.body;
    const now = dayjs().format("YYYY-MM-DD HH:mm:ss");
    const id = encryptedId ? await decrypt(encryptedId) : '0';
    const parent = dechiper ? await decrypt(dechiper) : await decrypt(encryptedParent);
    const creator = await decrypt(encryptedCreator);
...
export const listCollection = async (req, res) => {
  ...
  try {
    ...
    let query = db("collection_menu as a")
      .select('a.*', 'b.name')
      .innerJoin('menu as b', function() {
        this.on('a.col_parent', '=', 'b.id')
      })
...
export const listCollectionDetail = async (req, res) => {
  ...
  try {
    ...
    // Simple list without pagination
    if (!rowsPerPage) {
      let query = db("menu as a")
        .select('a.id', 'a.name', 'b.coldet_menu')
        .leftJoin('collection_det as b', function() {
          this.on('a.id', '=', 'b.coldet_menu')
        })
        .where(function() {
    ...
    let query = db("collection_det as a")
      .select('a.*', 'b.col_name', 'c.name', 'c.link', 'c.parent')
      .innerJoin('collection_menu as b', function() {
        this.on('a.coldet_colid', '=', 'b.colid')
      })
      .innerJoin('menu as c', function() {
        this.on('a.coldet_menu', '=', 'c.id')
      })
...
export const listCollectionMenu = async (req, res) => {
  ...
  try {
    ...
    const { role: encryptedRole, data,empid:empidDecrypt } = req.query;
    const role = encryptedRole ? await decrypt(encryptedRole) : '0';
    ...
    const dtMenu = await db("collection_det")
      .distinct('menu.*')
      .innerJoin('collection_menu', 'coldet_colid', 'colid')
      .innerJoin('menu', 'menu.id', 'coldet_menu')
      .innerJoin("user_access", 'access_menuid', 'coldet_menu')

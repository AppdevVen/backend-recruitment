...
export const uploadProfileImage = async (req, res) => {
  ...
  try {
    ...
    await uploadFile('file', 'template/background', filename);
    ...
    if (existData) {
      ...
      await trx('background_user')
        ...
        .where('bg_type', type)
        .update(updateData);
      await removeFile('template/background', existData.bg_filename);
    ...
    await trx.commit();
    return res.json({imageUrl:`${process.env.LINK_DOWNLOAD}/template/background/${filename}`});
  } catch (error) {
    await trx.rollback();
...
export const getProfileImages = async (req, res) => {
  ...
  try {
    ...
    return res.json({
      backgroundImage: background ? `${process.env.LINK_DOWNLOAD}/template/background/${background.bg_filename || background.bg_file}` : null,
      avatarImage: avatar ? `${process.env.LINK_DOWNLOAD}/template/background/${avatar.bg_filename || avatar.bg_file}` : null
    });
  } catch (error) {

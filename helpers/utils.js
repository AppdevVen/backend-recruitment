...
const secretKey = crypto.createHash('sha256').update('nest0425').digest(); 
...
export const encodeForUrl = async (message) => {
  return message.replace(/\+/g, '-').replace(/\//g, '_').replace(/=+$/, '');
};

export const decodeFromUrl = async (ciphertext) => {
...
export const encrypt = async (message) => {
  ...
  encrypted += cipher.final('base64');
  // Return IV + encrypted message, as we need the IV for decryption
  const result = iv.toString('base64') + '|' + encrypted;
  return await encodeForUrl(result); // Make it URL-safe
}
...
export const decrypt = async(ciphertext) => {
  try {
    ...
    const parts = restoredCiphertext.split('|');
    
    if (parts.length !== 2) {
      throw new Error('Invalid ciphertext format');
    }
    
    const [ivEncoded, encrypted] = parts;
    
    if (!ivEncoded || !encrypted) {
...
export const deletePDFFile = async () => {
    try {
      ...
      const pdfPath = fileArray[0] +'file\\';
